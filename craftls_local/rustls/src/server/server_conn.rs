...
/// Represents an alert to be sent after an unsuccessful acceptance.
pub struct AcceptedAlert {
    pub(crate) alert: crate::enums::AlertDescription,
    pub(crate) connection: ConnectionCommon<ServerConnectionData>,
}

impl AcceptedAlert {
    /// Write the TLS alert message into `wr`.
...
impl ServerConfig {
    ...
    /// Access the [`CryptoProvider`] for this configuration.
...
impl Acceptor {
    ...
    pub fn accept(&mut self) -> Result<Option<Accepted>, (Error, AcceptedAlert)> {
        let mut connection = match self.inner.take() {
            ...
            None => {
                ...
                let config = ServerConfig::builder()
                    .with_no_client_auth()
                    .with_cert_resolver(Arc::new(handy::AlwaysResolvesChain::new(
                        provider.key_provider
                            .load_private_key(pki_types::PrivateKeyDer::Pkcs8(pki_types::PrivatePkcs8KeyDer::from(Vec::new())))
                            .unwrap_or_else(|_| panic!("dummy key")),
                        CertificateChain(Vec::new())
                    )));
                let state = Box::new(hs::ExpectClientHello::new(Arc::new(config), Vec::new()));
                let core = ConnectionCore::new(state, ServerConnectionData::default(), CommonState::new(Side::Server));
                return Err((
                    Error::General("Acceptor polled after completion".into()),
                    AcceptedAlert {
                        alert: crate::enums::AlertDescription::InternalError,
                        connection: ConnectionCommon::from(core),
                    },
                ));
            }
        };
        ...
        let message = match connection.first_handshake_message() {
            ...
            Err(e) => {
                return Err((
                    e,
                    AcceptedAlert {
                        alert: crate::enums::AlertDescription::HandshakeFailure,
                        connection,
                    },
                ));
            }
        ...
        match hs::process_client_hello(&message, false, &mut Context::from(&mut connection)) {
            Ok((_, sig_schemes)) => Ok(Some(Accepted {
                connection,
                message,
                sig_schemes,
            })),
...
impl Debug for Accepted {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("Accepted")
            .field("sig_schemes", &self.sig_schemes)
            .finish()
    }
}
...
impl Accepted {
    ...
    pub fn into_connection(mut self, config: Arc<ServerConfig>) -> Result<ServerConnection, (Error, AcceptedAlert)> {
        if let Err(e) = self.connection.set_max_fragment_size(config.max_fragment_size) {
            return Err((e, AcceptedAlert {
                alert: crate::enums::AlertDescription::InternalError,
        ...
        let new = match state.with_certified_key(
            self.sig_schemes,
            ...
            Ok(new) => new,
            Err(e) => {
                let alert = cx.common.sendable_tls.is_empty().then(|| crate::enums::AlertDescription::HandshakeFailure).unwrap_or(crate::enums::AlertDescription::InternalError);
                return Err((e, AcceptedAlert {
                    alert,
                    connection: self.connection,
                }));
