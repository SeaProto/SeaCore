...
pub trait PacketKey: Send + Sync {
    ...
    /// Confidentiality limit for the underlying AEAD algorithm
    fn confidentiality_limit(&self) -> u64;

    /// Integrity limit for the underlying AEAD algorithm
...
#[derive(Clone, Copy, Debug)]
pub struct Suite {
    /// The underlying TLS 1.3 cipher suite.
    pub suite: &'static Tls13CipherSuite,
}

impl Suite {
    /// Return the cipher suite's ID
    pub fn suite(&self) -> CipherSuite {
        self.suite.common.suite
    }

    /// Return the keys for use with initial packets
    pub fn keys(
        &self,
        client_dst_connection_id: &[u8],
        side: Side,
