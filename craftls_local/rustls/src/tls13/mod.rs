...
impl Tls13CipherSuite {
    ...
    /// Return the QUIC-level algorithms for this suite, if available.
    #[allow(unsafe_code)]
    pub fn quic_suite(&self) -> Option<crate::quic::Suite> {
        self.quic.map(|_| crate::quic::Suite {
            // SAFETY: this is only used for cipher suites that are already static.
            suite: unsafe { core::mem::transmute(self) },
        })
    }
}
