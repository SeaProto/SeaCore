...
pub struct ClientConfig {
    ...
    /// If set, the extension order will be fixed according to this list.
    /// Extensions not in this list will be placed after.
    pub fixed_extension_order: Option<Vec<ExtensionType>>,

    /// If set, the cipher suite order will be fixed according to this list.
    /// Suites not in this list will be placed after.
    pub fixed_cipher_suite_order: Option<Vec<CipherSuite>>,

    /// Extra extensions to send in the client hello.
    pub extra_exts: Vec<ClientExtension>,

    /// Optional custom session ID to send in the client hello.
    pub custom_session_id: Option<Vec<u8>>,
}
...
impl ClientConfig {
    ...
    /// Access the [`ClientSessionStore`] for this configuration.
    pub fn session_storage(&self) -> &Arc<dyn ClientSessionStore> {
...
impl Clone for ClientConfig {
    fn clone(&self) -> Self {
        Self {
            ...
            fixed_extension_order: self.fixed_extension_order.clone(),
            fixed_cipher_suite_order: self.fixed_cipher_suite_order.clone(),
            extra_exts: self.extra_exts.clone(),
            custom_session_id: self.custom_session_id.clone(),
        }
    }
}
...
impl ClientConnection {
    ...
    pub fn new(config: Arc<ClientConfig>, name: ServerName<'static>) -> Result<Self, Error> {
        Ok(Self {
            inner: ConnectionCore::for_client(config.clone(), name, config.extra_exts.clone(), Protocol::Tcp)?.into(),
        })
    }

    /// Make a new ClientConnection with the given configuration, server name and ALPN protocols.
...
impl UnbufferedClientConnection {
    ...
    /// Make a new UnbufferedClientConnection with the given configuration, server name and ALPN protocols.
    pub fn new_with_alpn(
        config: Arc<ClientConfig>,
        name: ServerName<'static>,
        alpn: Vec<Vec<u8>>,
