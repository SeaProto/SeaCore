...
pub(super) fn start_handshake(
    ...
    let session_id = if let Some(ref custom_id) = config.custom_session_id {
        SessionId::new(custom_id)
    } else {
        match session_id {
...
fn emit_client_hello_for_retry(
    ...
    if let Some(order) = &config.fixed_extension_order {
        exts.sort_by_cached_key(|new_ext| {
            let typ = new_ext.ext_type();
            if typ == ExtensionType::PreSharedKey {
                return u32::MAX;
            }
            order
                .iter()
    ...
    let mut cipher_suites: Vec<_> = if let Some(order) = &config.fixed_cipher_suite_order {
        order
            .iter()
            .cloned()
            .filter(|&suite| {
                config
                    ...
                    .cipher_suites
                    .iter()
                    .any(|cs| cs.suite() == suite && cs.usable_for_protocol(cx.common.protocol))
            })
            .collect()
    } else {
        config
